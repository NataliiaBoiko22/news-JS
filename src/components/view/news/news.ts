...
import './news.css';
import { IntDataItem } from '../../../types/interface/interface';

class News {
    draw(data: IntDataItem[]): void {
        const news = data.length >= 10 ? data.filter((item: IntDataItem, index: number) => index < 10) : data;
        const fragment: DocumentFragment = document.createDocumentFragment();
        const newsItemTemp = document.querySelector('#newsItemTemp') as HTMLTemplateElement;
        news.forEach((item: IntDataItem, index: number) => {
